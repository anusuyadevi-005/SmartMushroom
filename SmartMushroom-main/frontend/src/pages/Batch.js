...
function Batch() {
  ...
  const fetchPrediction = async (daysSinceSpawn) => {
    // Allow 0 as a valid value (new batch); only skip if null/undefined or negative
    if (daysSinceSpawn == null || daysSinceSpawn < 0) {
    ...
    try {
      ...
      if (response.data && (response.data.expected_yield_kg !== undefined || response.data.expected_harvest_day !== undefined)) {
  ...
  useEffect(() => {
    ...
    // Fetch ML predictions using the configured growth days as batch age input
    fetchPrediction(Number(growthDays) || 0);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <form onSubmit={editingBatch ? handleUpdate : handleSubmit} className="space-y-6">
                      ...
                      <div className="flex gap-3">
                        <div className="flex-1">
                          <input
                            ...
                            className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 ${errors.batchId ? 'border-red-300 bg-red-50' : 'border-gray-300'
                              }`}
                          />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled={loading}
                        className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 ${errors.startDate ? 'border-red-300 bg-red-50' : 'border-gray-300'
                          }`}
                      />
                    ...
                    {/* Message */}
                    {message && (
                      <div className={`p-4 rounded-lg ${message.toLowerCase().includes('failed') || message.toLowerCase().includes('unable')
          ...
          <div className="lg:col-span-1">
            <div className={`bg-white rounded-xl shadow-lg transition-all duration-300 ${showPreview ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform -translate-y-4 pointer-events-none'
              }`}>
              <div className="p-6">
