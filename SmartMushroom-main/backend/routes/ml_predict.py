...
def predict_harvest():
    ...
    # Allow 0 (new/just-spawned batch); reject only if field is truly missing
    ...
    try:
        ...
        if batch_age < 0:
            return jsonify({"error": "days_since_spawn must be >= 0"}), 400
    except (ValueError, TypeError):
        return jsonify({"error": "days_since_spawn must be a number"}), 400
